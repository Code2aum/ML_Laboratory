...
input_neurons = 2 # Number of features in the data set
...
for i in range(epoch):
    ...
    d_output = Eo * outgrad # Error = Output(1 - Output)(Target - Output)

    # Error at Hidden layer
    Eh = d_output.dot(wout.T) # .T means transpose
    hiddengrad = sigmoid_grad(h_act) # How much hidden layer weights contributed to error
    d_hidden = Eh * hiddengrad

    wout += h_act.T.dot(d_output) * eta # Dot product of next layer error and current layer output
